  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950">
      <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Review Management</h1>
        <p className="mt-1 text-sm text-slate-600 dark:text-slate-300">Approve customer reviews for your products.</p>
        ...
        <section className="mt-6 rounded-2xl bg-white p-5 shadow-sm ring-1 ring-slate-200 dark:bg-slate-900 dark:ring-slate-700">
          ...
          <div className="space-y-4">
            {pendingReviews.map((review) => (
              <div key={review._id} className="rounded-xl border border-slate-200 p-4 dark:border-slate-700">
                <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    <p className="text-sm font-semibold text-slate-900 dark:text-slate-100">{review.productName || "Product"}</p>
                ...
                <p className="mt-3 text-sm text-slate-700 dark:text-slate-200">{review.review}</p>
              </div>
            ))}
          </div>
        </section>
