      ...
      <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6 sm:py-10">
        ...
        <section className="mt-6 rounded-2xl bg-white p-6 shadow-sm ring-1 ring-slate-200">
          ...
          <div className="space-y-4">
            {orders.map((order) => (
              <article key={order._id} className="rounded-2xl border border-slate-200 bg-slate-50/50 p-5">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  <div>
                    <p className="text-sm font-bold text-slate-900 tracking-wide">Order #{order._id.slice(-8)}</p>
                ...
                <div className="mt-4 rounded-xl border border-slate-200 bg-white p-4">
                  <div className="mb-2 flex items-center justify-between">
                    <p className="text-xs font-semibold uppercase tracking-wider text-slate-500">Order Items</p>
                    <p className="text-sm font-bold text-emerald-700">LKR {order.sellerTotal.toLocaleString()}</p>
                  </div>
                  <div className="space-y-2">
                  {order.sellerItems.map((item, idx) => (
                    <div key={`${order._id}-${idx}`} className="flex items-center justify-between gap-3 text-xs text-slate-700">
                  ...
                  <div className="rounded-xl border border-slate-200 bg-white p-3">
                    ...
                    <div className="flex items-center gap-2">
                      <select
                        ...
                        className="w-full rounded-lg border border-slate-300 px-3 py-2 text-xs font-semibold outline-none focus:border-slate-600"
                      >
